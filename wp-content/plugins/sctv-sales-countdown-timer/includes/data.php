...
class VI_SCT_SALES_COUNTDOWN_TIMER_Data {
	...
	public function __construct() {
		...
		$checkout = array(
			...
			'checkout_countdown_reset_type'   => 'day',
	...
	public function get_params( $name = "" , $language = '' ) {
		...
		return apply_filters( 'sctv_countdown_settings' . $name_t, $this->params[ $name_t ] ?? $this->params[ $name ] ?? false );
	}
	...
	public function get_current_countdown_language( $name = "", $language = '', $i = 0, $default = false ) {
		if ( empty( $name ) ) {
			return false;
		}
		if ( $default !== false ) {
			$result = $this->get_params( $name, $language )[ $i ] ?? $default;
		} else {
			$result = $this->get_params( $name, $language )[ $i ] ?? $this->get_default( $name )[0] ?? false;
		}
