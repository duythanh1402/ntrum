...
class VI_SCT_SALES_COUNTDOWN_TIMER_Frontend_Single_Product_Countdown {
		...
		switch ( $template_name ) {
			case 'single-product/sale-flash.php':
				if ( $this->position === 'before_saleflash' ) {
					...
					echo '<div class="woo-sctr-single-product-container woo-sctr-single-product-shortcode-' . esc_attr( $this->id ) . ( in_array( $this->pg_position, array(
							'left_countdown',
							'right_countdown'
	...
	public function woocommerce_single_product_summary() {
		...
		if ( $this->id && $this->position == 'product_image' ) {
			...
			$html = str_replace( '</figure>',
				'</figure><div class="woo-sctr-single-product-container"><div class="woo-sctr-countdown-timer-product-image-wrap woo-sctr-single-product-shortcode-' . esc_attr( $this->id ) . ( in_array( $this->pg_position, array(
					'left_countdown',
					'right_countdown'
	...
	public function woocommerce_available_variation( $variation_data, $parent, $variation ) {
		$wc_ajax = isset( $_REQUEST['wc-ajax'] ) ? wc_clean( wp_unslash( $_REQUEST['wc-ajax'] ) ) :'';
