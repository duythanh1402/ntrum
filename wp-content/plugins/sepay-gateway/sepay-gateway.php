...
function sepay_init_gateway_class()
{
    ...
    function sepay_api(WP_REST_Request $request): array
    {
        ...
        if ($order_total === $parameters['transferAmount']) {
            if (in_array($sepay_gateway->get_option('order_when_completed'), ['processing', 'completed'])) {
                $order->update_status($sepay_gateway->get_option('order_when_completed'));
            } else {
            ...
            $order_note = sprintf(
                ...
                wc_get_order_status_name(OrderStatus::COMPLETED)
            );
